...
@section('content')
          ...
          <form class="form-horizontal">
            <div class="box-body">
              <div class="form-group">
                ...
                <div class="col-sm-9">
                  <input type="hidden" name="issue_id" required value="{{$issue_header['seq_id']}}">
                  <select class="form-control" name="menu_id_category">
                    <option></option>
                    @foreach ($category as $cat)
                      @if( $cat['seq_id'] == $issue_header['menu_id_category'] )
                        <option value="{{$cat['seq_id']}}" selected>{{$cat['menu_name']}}</option>
                      @else
              ...
              <div class="form-group">
                ...
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="application_name" name="application_name" placeholder="Application Name" required value="{{$issue_header['application_name']}}">
                </div>
              </div>
              <div class="form-group">
                <label for="issue_summary" class="col-sm-3 control-label">Issue Summary</label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="issue_summary" name="issue_summary" placeholder="Issue Summary" required value="{{$issue_header['issue_name']}}">
                </div>
              </div>
              <div class="form-group">
                <label for="issue_detail" class="col-sm-3 control-label">Detail Description</label>
                <div class="col-sm-9">
                  <textarea class="form-control" rows="3" id="issue_detail" name="issue_detail" placeholder="Description ..." required>{{$issue_header['description']}}</textarea>
                </div>
              </div>
            </div>
            ...
            <div class="box-body">
              <div class="form-group">
                ...
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="solution_name" name="solution_name" placeholder="Solution Name" required value="{{$issue_header['solution_description']}}">
                </div>
              </div>
              ...
              <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                  @for ($i = 0; $i < count($issue_datasource); $i++)
                    @if($i == 0)
                      <?php 
                        $active = 'active'; $close = '';
                      ?>
                    @else
                      <?php $active = ''; $close = '<i class="fa fa-times-circle text-danger close-tab" aria-hidden="true"></i>';?>
                    @endif
                    <li role="presentation" class="{{$active}}"><a href="#table_{{$i}}" aria-controls="table_0" role="tab" data-toggle="tab">Table {{$i}} - {{$issue_datasource[$i]['table_name']}}</a>{!!$close!!}</li>
                  @endfor
                ...
                <div class="tab-content">
                  ...
                  <div role="tabpanel" class="tab-pane {{$active}}" id="table_{{$i}}">
                    <div class="form-group">
                      ...
                      <div class="col-sm-9">
                        <input type="text" class="form-control" id="table_datasource" name="datasource[{{$i}}][table_name]" placeholder="Solution Name" required value="{{$issue_datasource[$i]['table_name']}}">
                      </div>
                    </div>
                    <div class="form-group">
                      <label for="solution_datasource" class="col-sm-3 control-label">Data Query</label>
                      <div class="col-sm-9">
                        <textarea class="form-control query_editor" rows="8" id="solution_datasource" name="datasource[{{$i}}][data_source]" placeholder="SELECT * from..." required>{{$issue_datasource[$i]['data_log_query']}}</textarea>
                      </div>
                    </div>
                  </div>
                  @endfor
              ...
              <div class="form-group">
                ...
                <div class="col-sm-9">
                  <table class="table table-condensed table-bordered" id="paramater_data">
                    ...
                    @for ($i = 0; $i < count($issue_parameters); $i++)
                      <tr id="parameter_{{$i}}" class="paramater">
                        <td><input class="form-control input-sm" name="parameters[{{$i}}][parameter_label]" type="text" required value="{{$issue_parameters[$i]['param_label']}}"></td>
                        <td><input class="form-control input-sm" name="parameters[{{$i}}][parameter_field]" type="text" required value="{{$issue_parameters[$i]['param_field']}}"></td>
                        <td><input class="form-control input-sm" name="parameters[{{$i}}][placeholder]" type="text" value="{{$issue_parameters[$i]['placeholder']}}"></td>
                        @if($i > 0)
                          <td class="text-center"><i class="fa fa-minus-circle fa-2x text-danger" aria-hidden="true" onclick="removeParam({{$i}})"></i></td>
                        @else
                          <td></td>
                        @endif
                      </tr>
              ...
              <div class="form-group">
                ...
                <div class="col-sm-9">
                  <textarea class="form-control query_editor" rows="10" id="resolution" name="resolution" placeholder="" required>{{$issue_header['resolution_query']}}</textarea>
                </div>
              </div>
            </div>
  ...
  <script type="text/javascript">
    ...
    var currval = $('table tbody tr').length, tab_counter = 0;
